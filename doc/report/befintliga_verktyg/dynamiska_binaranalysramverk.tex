...
Binäranalysverktyget angr (alltid skrivet med små bokstäver) stödjer både
statiska analyser såsom dekompilering till pseudo-C-kod och dynamiska analyser
med hjälp av symbolisk exekvering. Användarens analyser utförs genom
Python-skript som interagerar med angrs API för att kontrollera en symbolisk
emulator skriven i Python för angr. Skript som använder angr kan användas för
\emph{reverse engineering}, sårbarhetssökning och kan även vara del av
exploateringsverktyg~\cite{angr_docs}. Tävlingen Cyber Grand Challenge
organiserades 2016 av DARPA.\@ I tävlingen skulle lag skriva helautomatiska
system som hittar, korrigerar och angriper sårbarheter i CTF-liknande uppgifter.
Det vinnande systemet, Mayhem, använde angr som symbolisk exekveringsmotor.
